...
$scpMesh = "/Game/BlackoutHunt/Art/SCP096/Skeletal/SK_SCP096.SK_SCP096"
$scpAnim = "/Game/BlackoutHunt/Art/SCP096/Skeletal/A_SCP096_Run.A_SCP096_Run"
$scpMaterial = "/Game/BlackoutHunt/Art/SCP096/M_SCP096.M_SCP096"
...
$hasScpMesh = Test-ManifestAsset -ManifestPath $manifestPath -AssetPath "BlackoutHunt/Content/BlackoutHunt/Art/SCP096/Skeletal/SK_SCP096.uasset"
...
if (-not $hasScpMesh -and $hasFab01Mesh) {
    $scpMesh = $fab01Mesh
    $scpAnim = $fab01Anim
    $scpMaterial = $fab01Material
}
elseif (-not $hasScpMesh) {
    Write-Warning "SCP096 mesh is not listed in the package manifest, and Fab fallback mesh is not available. Existing cooked binaries may still need the code fallback in the next binary build."
}
...
$lines.Add((New-JumpscareVariantLine -VariantId "SCP096" -DisplayName "SCP-096 No-Cook Scare" -Weight 0.70 -MinimumScareIntensity 0 -SkeletalMesh $scpMesh -RunAnimation $scpAnim -StaticMesh "" -Material $scpMaterial -LaunchSound $fallbackSound -VisualScale 0.42 -CloseScale 0.58 -LightColor "(R=1.000000,G=0.020000,B=0.000000,A=1.000000)" -FocusHeight 132.0 -CameraShakeIntensity 0.88 -FlashIntensity 0.62 -CameraJitterDuration 0.95))
...
Write-Host "  SCP096 visual source: $(if ($hasScpMesh) { 'SCP096 cooked mesh' } elseif ($hasFab01Mesh) { 'FabMonster01 cooked fallback' } else { 'existing binary fallback only' })"
