...
namespace
{
	...
	// Teleport detection is position-jump heuristic only. The game legitimately teleports players (locker exit,
	// stuck-zone reset, train-interior reset, final-escape relocation, roof reset, chair-sit) via TeleportPhysics,
	// and the intended exemption hook NotifyTrustedTeleport() is not yet wired at those sites -- so in enforce mode
	// (bh.AntiCheat 2) a non-zero weight would kick LEGITIMATE students for normal play. Keep teleport as log-only
	// (weight 0 -> never contributes to the kick budget) until every engine-teleport site calls NotifyTrustedTeleport.
	// Speed and Fly enforcement (the real movement cheats) are unaffected. [live-play-hardening]
	constexpr float BHACWeightTeleport = 0.0f;
