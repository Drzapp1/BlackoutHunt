...
void BuildElectricity(TArray<FBHRevisionQuestion>& Bank)
{
	static const FRevisionSpec MC[] = {
		...
		{TEXT("Components"), TEXT("A thermistor warms up near the boiler. What happens to its resistance?"), {TEXT("It decreases"), TEXT("It increases"), TEXT("It becomes infinite"), TEXT("It becomes a fuse")}, 0, TEXT("Thermistor resistance falls with temperature."), TEXT("Remember hot thermistor means easier current path."), TEXT("A thermistor's resistance decreases as temperature increases."), EBHDiagramType::Circuit, TEXT("thermistor: hot -> lower R"), 0.0f, 0.0f, {.LabelA=TEXT("thermistor"), .ShapeVariant=0}},
	...
	static const FRevisionSpec Match[] = {
		...
		// The two drag pieces must be textually distinct: GradeArrangement compares piece STRINGS per
		// slot, so two identical "lower R" pieces made every placement grade correct. Each piece now
		// names its own stimulus (still physically correct), so a swapped arrangement grades wrong.
		{TEXT("Components"), TEXT("Match the component behaviour."), {TEXT("Thermistor hot -> lower R as temperature rises; LDR bright -> lower R as light intensity rises"), TEXT("Thermistor hot -> higher R; LDR bright -> higher R"), TEXT("Fuse hot -> lower rating; lamp cool -> no resistance"), TEXT("Cell bright -> lower p.d.; switch open -> more current")}, 0, TEXT("Both thermistor and LDR can lower resistance when stimulated."), TEXT("Remember heat for thermistor, light for LDR."), TEXT("Thermistor resistance decreases as temperature increases; LDR resistance decreases as light intensity increases."), EBHDiagramType::Circuit, TEXT("thermistor hot lower R, LDR bright lower R"), 0.0f, 0.0f, {.LabelA=TEXT("sensors"), .ShapeVariant=0}},
...
void BuildElectricityExtension(TArray<FBHRevisionQuestion>& Bank)
{
	...
	static const FRevisionSpec Graph[] = {
		...
		{TEXT("Diode IV graph"), TEXT("An IV graph stays near zero current for reverse voltage, then rises sharply for forward voltage. Which component is this?"), {TEXT("Diode"), TEXT("Fixed resistor"), TEXT("Filament lamp"), TEXT("Thermistor")}, 0, TEXT("One-way conduction."), TEXT("Look at the asymmetry of the graph."), TEXT("A diode conducts in one direction, giving this asymmetric IV graph."), EBHDiagramType::IVGraph, TEXT("diode: one-way current"), 0.0f, 0.0f, {.LabelA=TEXT("forward only"), .XAxis=TEXT("V / V"), .YAxis=TEXT("I / A"), .ShapeVariant=2}},
