...
| SmartBasicInterfaces pack, `/Game/SmartBasicInterfaces` | `ABHButtonModule`, `ABHGameMode` station builders, and `ABHObjectiveStation` hard-load meshes/materials/sound under `Meshes`, `Materials`, `Sound`. | **Cooked into the package today, but license evidence is NOT yet recorded — verify before ship.** Demo maps/blueprints are now excluded via `DirectoriesToNeverCook` (`/Maps`, `/Demo`). | ⚠️ **License-evidence gate (pending):** record source listing URL, license type, purchase/entitlement proof, and required attribution before the next classroom distribution. Add attribution to the shipped credits and a notice block in `Docs/THIRD_PARTY_NOTICES.txt`. If the pack's EULA does not permit incorporation into a distributed build, remove its `DirectoriesToAlwaysCook` entry and gate the runtime behind a soft-ref fallback. |
