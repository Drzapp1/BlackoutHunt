...
# Pass 4 — cross-system races, backend security, and build validation (2026-06-05)

The last genuinely-uncovered categories: **frame-coincident / reconnect race scenarios** traced end-to-end (the integration bugs single-system audits structurally miss), a **security review of the server-side AccountBackend** (Node.js + Apps Script — the C++ client was audited but never the server), and an actual **incremental editor build** to validate that the fixes compile.

## Pass-4 fixed & committed

- **Tutorial Counterplay marker contradicted its own lesson** (`BHTutorialDirector.cpp:745`) — the step hides the AI student in a locker to teach the Teacher that concealed students vanish, but the objective beat kept a "STUDENT" marker pinned to the hidden student. Clear the beat while they're hidden. **Commit `7094a1e`.**
- **Apps Script feedback sink: spreadsheet formula injection** (`Tools/AccountBackend/apps-script/Code.gs`) — the **live** feedback/telemetry sink the shipped game POSTs to `appendRow`'d raw, attacker-controllable client strings into the owner's Google Sheet; a `=`/`+`/`-`/`@`-prefixed field becomes a live formula executing in the owner's authenticated Google session on open. Added `safeCell_`/`safeRow_` neutralization. **Commit `1799f6a`.** *(Owner must redeploy the Apps Script to activate; `SCRIPT_VERSION` bumped to confirm.)*
- **`TickRoundTimer` defeats the reconnect-grace defer** (`BHGameMode.cpp:13327`) — `Logout` deliberately defers a HunterWin while dropped survivors are within their 120 s reconnect grace (so a classroom Wi-Fi blip that drops the last survivors together doesn't instantly hand the Teacher the win), but `TickRoundTimer`'s Hunt branch re-checks `CountAliveSurvivors() <= 0 → EndRound` every second with **no such guard**, ending the round ~1 s later anyway. Each function is internally consistent; together they disagree, so the grace hold is effectively dead. Fix mirrors the Logout defer, resolving once grace lapses **or** the Hunt `RemainingTime` expires (no-hang backstop, never overruns the round timer). **Commit `cdd55db`.**

## Pass-4 reported, not fixed

**Reconnect / travel races** (the rest of scenario set; all bounded, Low–Med):
- **Reconnect lands in an already-resolved round** (`BHGameMode.cpp` PostLogin reconnect branch) — within `EndRound`'s 8 s pre-travel window, a dropped survivor can be restored *Alive* into a `*Win` round. The result is **not** flipped (EndRound latched the phase; the ticker is replaced by `ResetRoundByTravel`), but they're briefly a capturable survivor in a decided round and miss the result card. Recommend restoring banked points but forcing `LifeState=Captured` + sending `ClientRecordRoundResult` when reconnecting into a `*Win` phase. Low-Med.
- **Train-Intermission reconnect skips the train role/ready normalization** (`BHGameMode.cpp` vs `RestorePlayersAfterTravel`'s train block) — a reconnecting passenger isn't force-Ready'd/Survivor-normalized; pawn is still granted (no strand). Low.
- **Drop before `ClientReceiveReconnectToken` arrives** → no token → grace-reconnect impossible (falls to spectator). Documented identity-safety tradeoff; could issue the token in `InitNewPlayer`. Low.
- Scenarios verified **PASS**: travel-while-reconnect-in-flight (token carried, no duplicate entry), reconnect during FinalEscape (freeze derives from replicated flags, no strand), Logout during the 8 s window (no double-EndRound/flip), token match-priority/stale-token-hijack, double-restore (mutually exclusive branches; non-additive writes).

**Frame-coincident gameplay events** — **all 10 modeled scenarios PASS.** The single-threaded server (synchronous field writes that take effect before the next handler), `EndRound`'s Win-phase re-entry guard, every capture/escape call-site re-checking `IsAliveSurvivor()` after the LifeState flip, and a uniform `CountEscapedSurvivors()` tie-break across all resolution sites make two-survivors-escape, escape+capture-same-tick, capture-vs-timer, multi-board, gate-unlock-race, infection+escape, and director-scare-on-transition all resolve to a single correct winner. One Low **defense-in-depth** note: `NotifySurvivorCaptured`/`NotifySurvivorEscaped` don't themselves re-check out-of-play state (correct today via callers; add a guard if a new caller is ever introduced).

**AccountBackend security** (`Tools/AccountBackend/`) — the server-side scaffold. The shipped game uses the Apps Script sink (fixed above) and ships with external login **off** (`bEnableExternalAccountLogin=False`, `BackendBaseUrl=""`), so the `server.mjs` findings bite only if the self-hosted Node backend is deployed — but they're real and should be fixed before any public deployment:
- ★ **Unauthenticated device-poll leaks the session token** (`server.mjs:1060`) — `GET /auth/device/<id>` returns the bearer token + PII to anyone who knows the (browser-history/log-leaked) device id → account takeover. Require a client-held one-time poll secret; single-use + expire the entry. **High.**
- ★ **Unauthenticated `POST /setup/google` overwrites `.env` + OAuth creds** (`server.mjs:1043`) — on a `0.0.0.0` deploy, anyone can repoint the server's OAuth app. Gate behind admin auth / loopback. **High.**
- **No session expiry/revocation; `sessions`/`deviceAuth` never pruned** (`server.mjs:150`) — unbounded token lifetime + memory growth. **Medium.**
- **`X-Forwarded-For` trusted unconditionally** (`server.mjs:302`; the bundled Caddyfile doesn't sanitize XFF) — rate-limit bypass + IP/log spoofing. Use a trusted-proxy allowlist. **Medium.**
- **OAuth has no CSRF cookie / no PKCE; `state` reusable for its TTL** (`server.mjs:709`) — login-CSRF / state replay. **Medium.**
- **`ADMIN_TOKEN` + `state-secret` live in plaintext on disk** (`.env`, `data/state-secret.txt`) — **confirmed git-ignored & untracked** (not a repo leak; documented dev-scaffold posture), but rotate them since they've appeared in a working tree. **Medium.**
- `loadPlayers()` unguarded `JSON.parse` (corrupt file crashes startup) + non-atomic `savePlayers` (Low); per-request `existsSync` + full-file reads on the admin path with no log rotation (Low DoS amplification); login-error page reflects `error.message` into HTML unescaped (Low). 

## Pass-4 build validation

To de-risk the deliverable (the fixes were authored static-only), I ran the project's own `Tools/Build-Editor.ps1` (`BlackoutHuntEditor Win64 Development`, the OOM-safe `-NoXGE -NoUBA -MaxParallelActions=1` invocation) against `D:\UE_5.7`. **Both builds reported `Result: Succeeded`** — UnrealHeaderTool processed the module with 0 errors (the new non-UPROPERTY members in `BHDoor.h`/`BHSlidingGate.h` don't perturb reflection), and `UnrealEditor-BlackoutHunt.dll` compiled and linked clean (only pre-existing engine deprecation warnings, none from the changed code). The full pass-1→pass-4 set of **15 game-code fixes compiles and links**; the 2 non-code fixes are config (`DefaultGame.ini` AlwaysCook) and the Apps Script backend (`Code.gs`). *So every C++ change in this branch is build-validated, not just statically reasoned.*

*Pass 4 generated by 1 orchestrator + 4 parallel auditors (2 race-scenario + 1 backend-security + the build) on 2026-06-05.*

---
---

# Pass 5 — clearing the reported-but-unfixed backlog (2026-06-05)

New directive from the owner: *"You may fix the bugs you found, no matter what danger level, just keep it
reversible."* This pass works through the ~50 items that passes 1–4 reported but deliberately left unfixed,
applying every one that is **safe to change blind and reversible** (each is additive/guarded and cleanly
`git revert`-able), and documenting — with rationale — the ones that genuinely should *not* be changed blind.

Every fix was re-verified against the real code before editing (the pass-1 audit caught several agent
false-positives this way), grouped into reversible commits, and the C++ batches were **build-validated**
(`Tools/Build-Editor.ps1`, `Result: Succeeded`) before committing.

## Pass-5 fixed & committed (~40 issues across 7 commits, off `bugfix/deep-audit-2026-06-04`)

**Batch A — safe defensive C++ (`12b2cc8`, build-validated):**
boundary-aware `AppendMaxPlayersOption` (colliding option keys no longer false-match); `ResolveTarget`
preferred-branch aligned to alive-survivor + not-in-locker; null-guarded the dead `HandleSurvivorThreat`;
`BHNoiseDecoy::EndPlay` clears its repeating timer; `ExitGate`/`JumpscareMonster` skip cosmetic ticks /
synchronous mesh loads on a dedicated server; Tester locker prompt gated to an occupied locker (kills a dead
prompt); EMSpectrum loops/divisor driven from the band table (`UE_ARRAY_COUNT` + `static_assert`); constant-
time credential MAC compare; login-poll **deadline cap** (an abandoned OAuth flow stops polling); saturating
XP/rounds add; automation scratch-world rooted against in-test GC; shared `BHDoorOpenYawDegrees` constant;
lesson-preset bot cap derived from `MaxPlayers-1`; floored a negative spawn `PlayerIndex`; **defense-in-depth
out-of-play guards** in `NotifySurvivorCaptured`/`NotifySurvivorEscaped` (the win/loss path).

**Batch B-D — round-flow / education / atmosphere (`ddf6c6a`):**
`TickRoundTimer` FinalEscape backstop; `SetBotCount` no longer flips `bBotMode` mid-round (kept the live-bot
count re-clampable); manager-less final-escape fallback logs loudly; team-vote tally sized to the real choice
count (was a hard `[4]` → a >4-choice question could soft-lock); train-bonus terminal records the rate-limit
timestamp only after the reject guards + removed the dead correction-hold and its misleading "retry"
message; **per-player** CCTV-glitch cooldown (was director-wide → one survivor's cue suppressed everyone's);
presence decay made time-based (was a flat per-tick drop at a mode-dependent interval).

**Batch E-H — bot memory / determinism / replication / flashlight (`faebf39`, build-validated):**
bot world-memory snapshot now **merges** the bot's own just-seen hunter/survivor instead of clobbering it;
`BlackoutFlashlightStrengthScale=0` honoured (was clamped to a 0.02 floor); `bh.RoundSeedOverride` cvar +
RoundSeed-folded revision-question seed make a logged round reproducible for replay/QA; `RuntimeMeshProp`
re-applies serialized visuals in `BeginPlay` (level-placed instances render on the host); `ENUM_CLASS_FLAGS`
on the `EBHWarmupStep` flag enum.

**Batch I/K/M + config (`04615da`, build-validated):**
`TravelPlayerProgress` bounded (prunes pending marks past grace + a generous cap that only evicts old,
non-pending, non-current snapshots); HUD survivor-only TEACHER/FEAR/DREAD meters role-gated + panel re-flowed
(no more dead-at-0 meters in the Teacher/Monitor tutorials); a reconnect into an already-resolved round is
marked out of play + sent the result card (no capturable survivor in a decided round);
`r.StaticMeshLODDistanceScale` de-inverted (was 1.45→1.00 down the buckets; now rises with quality).

**Batch N — self-hosted AccountBackend hardening (`254b1cf`, `node --check` validated):**
gated `POST /setup/google` behind loopback/admin-token (was an unauthenticated `.env` overwrite); single-use
device-poll (a leaked device id can't replay the session token); X-Forwarded-For trusted only from a loopback
proxy; periodic prune of sessions/deviceAuth/pendingStates; `loadPlayers` tolerates a corrupt file; atomic
`savePlayers`; HTML-escaped OAuth error page.

**Batch O — verifier + docs (`3a0632f`):**
classroom-package verifier gained the missing `__ExternalActors__/__ExternalObjects__` rules for the
Hider/Hunter/Downloaded roots; corrected two stale `Docs/ASSETS.md` cook claims (Maps *is* AlwaysCook'd; the
SmartBasicInterfaces `Demo/` subtree is *not* — and cannot be — NeverCook-excluded); added the real `Tab`
(question cursor) and `N` (node marker) bindings to the controls list.

## Pass-5 deliberately NOT fixed (report-only) — and why

These are the items where changing blind would be unsound even though it's "reversible." Reversibility makes a
*mistake* recoverable; it doesn't make a *wrong design decision* or an *untestable behavior change* correct.

- **Design decisions (need the owner's intent):** solo live round with zero hunters (degenerate `MinPlayers=1`
  misconfig — bot vs block vs practice is a product call); AFK hall-monitor blocking the revision exit (no
  clean "AFK" signal; monitors are *intended* to count toward 70/50); `MajorityNeeded` counting silent
  bystanders (changing it alters the team-consensus mechanic); train-intermission reconnect normalization
  (reconnect-restore vs train-normalize is a design call).
- **Movement tuning / geometry (need playtesting):** anti-camp burst decay + the 150 u/s threshold; the
  special-move forward-clearance probe height; prone auto-stand under a low ceiling; the sprint-exhaustion
  hysteresis (client 0.08 vs server 0.05 — naively unifying either opens a sub-start-gate sprint or drops the
  hysteresis). Blind movement changes risk feel regressions / exploits.
- **Dormant or non-defect:** the StateTree-only bot fallbacks/watchdog (`bUseStateTreeAI=False` by default);
  the bot sentinel "mismatch" (`-999`/`-9999` are two *internally consistent* conventions); the `Tan`-domain
  cone risk (the two designer-facing cones are already `Clamp`ed); `SubmitNumericAnswer`'s hardcoded revision
  flag (Calculation questions only exist in revision mode, so the hardcode is always correct).
- **Working-today, risky-to-touch:** `BHFlickerLight`'s cross-machine-time early-clear (the authoritative timer
  already ends the burst correctly — rewriting the client countdown risks the one path that works); the ~40
  unchecked `GetWorld()` derefs in Server RPCs (unreachable on the authority; a 40-site mechanical sweep adds
  churn + risk for zero behavior change); the Slate sticky-status / host-action-`TAttribute` items (a
  freshness-timestamp refactor across 30+ setters or a Slate rework that needs *visual* testing); the
  listen-server hit-stop (gating it removes the host's own hit-stop — a tradeoff).
- **Needs a test cook / licensing decision (the owner's, + a cook the editor OOMs on here):** forbidding the
  runtime-soft-loaded `ContainersHouseCH` / `BFHorror` / other imported-pack roots in the verifier (a baked
  `.umap` may legitimately reference them — fail-gating blind could break a valid package); the
  `M_BH_DiagramRT` AlwaysCook entry (the asset is *generated* by `BHCreateDiagramMaterialCommandlet`, not yet
  on disk — run the commandlet then add the entry together).
- **Coordinated client change / OAuth-flow rework (untestable here):** the full per-client device-poll secret
  (the C++ client would have to send it); OAuth PKCE + CSRF cookie. The server-side single-use mitigation
  above is the safe, contained half.
- **Owner actions (not a code change):** rotate the on-disk `ADMIN_TOKEN` / `state-secret` (they appeared in a
  working tree); record the SmartBasicInterfaces `Demo/` license evidence. **Do not** delete the stray
  `D:\BlackoutHunt\BlackoutHunt\` duplicate tree from here — it's untracked, so a delete is *not* git-reversible;
  left for the owner.

## Pass-5 build validation

Each C++ batch was built with `Tools/Build-Editor.ps1` (`BlackoutHuntEditor Win64 Development`, the OOM-safe
`-NoXGE -NoUBA -MaxParallelActions=1` invocation) and reported **`Result: Succeeded`** before its commit
(Batch A, E-H, and I/K/M each built clean; one transient failure — `BHObjectiveStation` reaching the protected
`RoundSeed` — was fixed with a `GetRoundSeed()` getter and rebuilt green). The backend change is
`node --check`-validated; the verifier/docs/config changes are non-compiled. A final full-branch build was run
to confirm the cumulative state links clean.

*Pass 5 generated on 2026-06-05. Branch `bugfix/deep-audit-2026-06-04`; 7 fix commits (A, B-D, E-H, I/K/M+L,
N, O) on top of the pass-1→4 work.*
