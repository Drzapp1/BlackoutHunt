...
**Host lobby customization (extension):** the preset additionally carries a **starting difficulty** + **min/max difficulty band** (the live selection — including the adaptive picker — is clamped to that band, so a Hard-only or Easy-only revision is possible on the existing 3-tier axis); an **exact question set** (either a per-question `"enabled": false` flag in `QuestionBank.json`, or a named id-list file under `Saved/ClassroomPresets/QuestionSets/<id>.json` referenced by `QuestionSetId` — selection then draws only from that set, with a safe fall-through if a present station's topic is uncovered); a **variable-length map route** (`MapRoute`, ordered, repeats allowed — drives the per-stage map and the train-intermission destination, and doubles as the "maps used" pool; empty = the default Facility→Substation→Foggrounds); and **procedural layout knobs** (`LayoutSeed`, `BreakerCount` 3–12, `LayoutDensity` 50–160% partition/cover, and `bForceProcedural` which runs the runtime generator even when authored `.umap`s are enabled — seed/density only affect the procedural Facility). All of these serialize into the launch URL (`?BHStartDifficulty/?BHMinDifficulty/?BHMaxDifficulty/?BHQuestionSet/?BHMapRoute/?BHLayoutSeed/?BHBreakerCount/?BHLayoutDensity/?BHForceProcedural`) and are **re-emitted on every stage hop** (`BuildTravelOptionsForLevel`/`ResetRoundByTravel`) so they survive train intermissions, and are applied live in-lobby by `ApplyLessonPreset`. Reachable today by editing the preset/bank/set JSON; the in-menu host controls (`ABHPlayerController::*Menu* / ApplyAdvancedLessonOptionsForMenu`, staged into `BuildCurrentLessonPresetSnapshot`) drive the same fields.
...
### 11.4 Comfort
Three independent scalars apply on the client: **reduced jumpscares** (softens the close-up, skips hitstop and rumble, scales the scare to 0.45), **reduced flash** (quarters flash, skips the hard black-blink; lights and the monster mute their strobe to 0.25), and **reduced camera shake** (quarters shake and disables the FOV punch; also quarters the train-ride sway). Captions and high-contrast HUD round out the accessibility set. A **hide very-close players** toggle (default off) fades out other players' bodies locally when they crowd right against the camera (e.g. everyone bunched around the same node/breaker) so they don't block the view — local cosmetic only, with hysteresis, and it never hides the Hunter. Defaults are configurable and player-overridable.
...
- **"The train is moving" camera sway:** while in transit, a looping camera shake (`UBHTrainSwayCameraShake`, driven by `ABHPlayerController::TickTrainMotion` off the replicated `EBHTrainPhase`) gently rocks the view — a side-to-side roll, vertical rail bob, and a slow swell so it "breathes". It's **stronger on the roof** (the exposed easter-egg vantage) and respects the **reduced camera shake** comfort option (¼ scale). Camera-only; it never touches the pawn body or aim.
- **Interior lighting & boot sequence:** small, frequent warm "service light" modules (`ABHTrainServiceLight`) line the carriage centreline for a comfortable, flashlight-free baseline ("half on, not perfect"). On load each module **boots on red "backup power" for ~3.5 s, then ramps to a warm mains white** so the car feels like it powers up around you. The boot/look run client-side off `BeginPlay`, so they need no replicated state; the old teal cove flicker-lights are kept but dimmed to a subtle accent.
- **Per-player roof-light breaker (easter egg):** a hidden breaker on the roof (`ABHTrainRoofBreaker`, trackable with the **N** node-marker key) toggles a row of cool roof service lights for the interacting player **only** — client-local lighting, no shared state. See `Docs/EASTER_EGGS.md` egg #5.
- **Roof hang-out (easter egg):** the roof is a stargazing vantage — an emissive starfield + festoon lights (`ABHRoofStarfield`), calm decorations (telescope, deck seating, antennas, vents), and a short **parkour course** ending in a finish gate (`ABHTrainRoofParkourGate`) that grants the **Roof Runner** achievement + XP. A roof down-hatch and the **O** "reset to train" key always get a stuck player back inside (`ABHGameMode::ResetPlayerToTrainInterior`, train levels only). Built by `BuildTrainRoofExtras`, shared by the intermission and lobby roofs.
- **Train as the pre-game lobby:** hosting now boots into a parked **7-car lobby train** (`BuildTrainLobbyLevel`, `?BHLobby=1`) with a large social/seating lounge, so joining players gather and see each other while they wait — instead of idling in the empty hunt map. It has no auto-advance; on ready-up / host force-start the lobby **departs to the chosen first hunt level** (`TravelFromLobbyToFirstHunt` → `?BHAutoPrep=1`, which starts the normal warmup on arrival). The roof extras + reset key apply here too.
...
- **Substation** — a larger 12-player utility/industrial map, **reworked for playability (2026-06)**: a continuous **west cable gallery**, an open **central transformer hall** (8 banks flanking a clear sightline cross) ringed by **flanking rooms that each have 2–3 looped entrances (no dead-ends)**, and a **clean spawn muster foyer** just west of the platform (survivors no longer spawn on top of the station furniture). ~8 breakers, ~16 objective stations, 3 circuit-gated shutters, terminals, lockers/squeeze-pockets for hiding, plus **4 prone-only crawl tunnels that cut *through* the interior dividers as survivor-only shortcuts** — a sheltering prone survivor is uncapturable and the Teacher is ejected, so the hunter must loop to a door while the hider slips across or jukes between the two mouths (the old crawls were free-standing mid-room "sitting ducks"; this follows the Foggrounds wall-cut + lintel pattern). The wall/room layout is generated and connectivity-verified by `Tools/MapAnalysis/substation_proposed.py` (0 single-entrance rooms, down from 14 in the old grid; all 4 crawls confirmed to join two distinct rooms).
...
- **Progress:** rounds played, hunter/survivor wins, escapes, XP, plus the achievement counters added in **schema v3** (lifetime captures, locker hides, perfect chains). Saved locally under `Saved\Account` (atomic writes with `.bak`/`.tmp` rotation, forward-compatibility lock). Synced to the backend with a bearer token when enabled.
- **Cosmetics** (`BHCosmeticUnlocks`) — **local, no gameplay effect**, gated by **XP or achievements**: outfits (Casual/Worker free, Adventurer @100 XP, the rest achievement-gated), shirt colors (8 free + achievement-gated prestige tints), headwear (0–750 XP), and achievement-only nameplate **titles** + **emblems**. XP loss / a missing achievement can't keep a locked item equipped.
- **Achievements** (registry in `BHAccountSubsystem.cpp`, ~49 total incl. a heavy secret share; see `Docs/EASTER_EGGS.md`): a 2026 pass added a long tail of dedication / skill / education / easter-egg goals on top of the originals, rewarding titles, emblems, and re-gated outfits. Countable ones derive from lifetime stats; event/secret ones grant via the `ClientGrantAchievement` RPC or a client-local `UnlockAchievement`; two capstones (**Honor Graduate** = all standard, **Perfectionist** = all) roll up automatically from the registry.
...
- **Accessibility / comfort:** reduced jumpscares, reduced flash, reduced camera shake, captions (on by default), high-contrast HUD, colorblind palette, hide very-close players, HUD scale/opacity, and a POV-animation intensity with a reduced-motion scale. Scare intensity itself is tunable per lesson (0 = no scares).
...
- **Comfort defaults:** reduced jumpscares/flash/shake off, captions on, high-contrast off, hide very-close players off, HUD scale 1.0, opacity 1.0, all HUD elements on.
